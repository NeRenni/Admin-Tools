...
function main()
    ...
    if check_server() then
        ...
        sendChatMessage("Скрипт работает только на серверах PayDay RolePlay!")
        thisScript():unload()
    end
                ...
                if f then
                    ...
                    if updateversion ~= thisScript().version then
                        lua_thread.create(function()
                            local dlstatus = require('moonloader').download_status
                            local color = -1
                            sendChatMessage('Найдено обновление: '.. thisScript().version ..' -> '.. updateversion ..'! Загрузка..')
                            wait(250)
                            downloadUrlToFile(updatelink, thisScript().path,
                                function(id3, status1, p13, p23)
                                    if status1 == dlstatus.STATUS_DOWNLOADINGDATA then
                                    elseif status1 == dlstatus.STATUS_ENDDOWNLOADDATA then
                                        sendChatMessage('Загрузка окончена. Скрипт обновлен на версию '.. mc .. updateversion)
                                        goupdatestatus = true
                                    ...
                                    if status1 == dlstatus.STATUSEX_ENDDOWNLOAD then
                                        if goupdatestatus == nil then
                                            sendChatMessage('Скрипт не смог обновится на версию '.. updateversion)
                                            update = false
                                        end
                                    end
                                end
                        ...
                        sendChatMessage('Обновлений не найдено')
                    end
                end
            else
                print('Не удалось получить JSON таблицу', "Ошибка")
                sendChatMessage('Обновление не удалось')
                update = false
            end
        end
    end)
